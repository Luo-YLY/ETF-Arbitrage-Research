...
均值回复页面的收盘研究口径已从“策略业绩展示”调整为“偏离收敛机制诊断”。主要指标为
收敛交易数、收敛率、30/60/300秒收敛率、收敛时间中位数与P90、MAE/MFE，以及不同往返
成本下的净价差捕获。原累计收益和最大回撤仅作为收敛研究指数的辅助字段，不解释为账户净值；
Sharpe不再作为控制台核心指标。
...
- 标准化Redis模式尚未直接读取早期 `sz_quotation.py` 的日期 Hash 和 `159915.SZ` 原始字段；
...
模拟行情另提供“Redis最新价初始化”混合模式。系统通过 `SZ_REDIS_*` 环境变量只读访问日期
Hash，批量获取 ETF 与当日 PCF 全部实物成分股的 `closepx`，把这些价格作为模拟器首个快照
的锚点。该模式不会把最新成交价扩展解释为真实盘口：Bid/Ask、五档深度、冲击和后续价格路径
仍是合成数据，并在控制台状态中明确标识。任何必需成分股缺失、非数字或价格不为正时均拒绝启动。
...
1. **原始Redis只能用于价格锚定。** 当前可以消费 `.SZ` 日期 Hash 的 `closepx` 初始化模拟器，
   但真实可执行模式仍需要标准化的ETF与成分股多档盘口。
